...
interface ImageCropperProps {
  imageUrl: string;
  aspectRatio: number;
  onSave: (base64: string, caption?: string) => void;
  onCancel: () => void;
  isUploading: boolean;
}
      ...
      <div className="w-full max-w-lg bg-slate-800 border border-slate-700 rounded-lg shadow-2xl p-6 relative">
        ...
        {aspectRatio !== 1 && (
            <textarea
                ...
                className="w-full bg-slate-700 border border-slate-600 text-slate-100 rounded-lg p-3 focus:ring-rose-500 focus:border-rose-500 resize-none mt-4"
                rows={2}
            />
        )}
